...
#if 1
static StaticTask_t rtc_thread_memory;
#if defined(__ARMCC_VERSION)           /* AC6 compiler */
                static uint8_t rtc_thread_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t rtc_thread_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.rtc_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
TaskHandle_t rtc_thread;
void rtc_thread_create(void);
static void rtc_thread_func(void *pvParameters);
...
const rm_freertos_port_parameters_t rtc_thread_parameters =
{ .p_context = (void*) NULL, };
...
void rtc_thread_create(void)
{
...
#if 1
    rtc_thread = xTaskCreateStatic (
#else
                    BaseType_t rtc_thread_create_err = xTaskCreate(
                    #endif
                                    rtc_thread_func,
                                    (const char*) "rtc_thread", 1024 / 4, // In words, not bytes
                                    (void*) &rtc_thread_parameters, //pvParameters
                                    1,
#if 1
                                    (StackType_t*) &rtc_thread_stack,
                                    (StaticTask_t*) &rtc_thread_memory
#else
                        & rtc_thread
                        #endif
                                    );
    ...
    if (NULL == rtc_thread)
    {
        rtos_startup_err_callback (rtc_thread, 0);
    }
#else
                    if (pdPASS != rtc_thread_create_err)
                    {
                        rtos_startup_err_callback(rtc_thread, 0);
                    }
                    #endif
